	...
	while(1)
	{
		...
		switch(opt)
		{
			...
			case	6	:	/* Delete element at position */
								printf("Enter position : ");
								scanf("%d",&pos);
								if(!(pos^(list_size-1)))/* Delete at end */
									delete_node_at_tail(root);
								else if(!pos)/* Delete at start */
									root=delete_node_at_head(root);
			...
			case	9	:	/* Print list with full node details*/
								print_list_full_node(root);
								break;
