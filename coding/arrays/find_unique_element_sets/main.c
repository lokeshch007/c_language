...
{
	arr->set=(int*)realloc(arr->set,sizeof(int)*size);
	if(!arr)
	{
		printf("relloc() error\n");
		exit(1);
...
int main(void)
{
	...
	for(int i=0;i<size-1;i++)
	{
		...
		if(temp^array[i+1]) /* Not equal */
		{
			...
			{
				push(array[i]);
				for(int j=0;j<=same_ele_occur;j++)
				{
					pop(&data);
					add_ele_to_array(&sets_array[sets_array_index],j+1,data);
					printf("\nsetcount:%d\n",sets_array[sets_array_index].setcount);
				}
				same_ele_occur=0;
			}
	...
	if(same_ele_occur>0)
	{
		...
		{
			pop(&data);
			add_ele_to_array(&sets_array[sets_array_index],same_ele_occur+1,data);
		}
		same_ele_occur=0;
	}
	else
